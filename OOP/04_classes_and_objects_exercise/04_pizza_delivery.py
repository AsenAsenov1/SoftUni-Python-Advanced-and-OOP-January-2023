...
o	If we already have this ingredient in our pizza, increase the ingredient quantity with the given one and update the
pizza price by adding the ingredient price for the given quantity
...
class PizzaDelivery:
    ...
    def make_order(self):
        ...
        self.ordered = True
        ingredients_quantity = [f"{key}: {value}" for key, value in self.ingredients.items()]

        return f"You've ordered pizza {self.name} prepared with {', '.join(ingredients_quantity)} and the price will be " \
               f"{self.price}lv."
