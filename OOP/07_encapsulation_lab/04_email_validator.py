...
class EmailValidator:
    ...
    def __is_mail_valid(self, mail):
        for current_mail in self.mails:
    ...
    def validate(self, email):
        info = email.split("@")
        name = info[0]
        mail, domain = info[1].split(".")

        name_valid = self.__is_name_valid(name)
        mail_valid = self.__is_mail_valid(mail)
        domain_valid = self.__is_domain_valid(domain)

        if all([name_valid, mail_valid, domain_valid]):
            return True
        return False
